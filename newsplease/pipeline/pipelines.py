...
class MySQLStorage(object):
    ...
    compare_versions = ("SELECT * FROM scrappednews_current WHERE url=%s")
    insert_current = ("INSERT INTO scrappednews_archive(source,scrapetime,hyperlink,heading,article,author,imageurl,publishtime) VALUES(%(source)s,%(scrape_time)s,%(hyperlink)s,%(title)s,%(title)s,%(author)s,%(img_url)s,%(date_publis)s)")

    insert_archive = ("INSERT INTO scrappednews_archive(source,scrapetime,hyperlink,heading,article,author,imageurl,publishtime) VALUES(%(source)s,%(scrape_time)s,%(hyperlink)s,%(title)s,%(title)s,%(author)s,%(img_url)s,%(date_publis)s)")

    delete_from_current = ("DELETE FROM scrappednews_current WHERE id = %s")
    ...
    def __init__(self):
        ...
        # Establish DB connection
        # Closing of the connection is handled once the spider closes
        self.conn = psycopg2.connect(dbname=self.database["db"],
                    user=self.database["username"],
        ...
        if old_version is not None:
            ...
            try:
                ...
                self.conn.commit()
                    ...
                    psycopg2.IntegrityError, TypeError) as error:
